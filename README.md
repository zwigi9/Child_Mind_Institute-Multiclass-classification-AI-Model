...
Earlier, in our first attempt we used `RandomForestRegressor` to impute missing classes in the target column ([here](https://github.com/zwigi9/Child_Mind_Institute-Multiclass-classification-AI-Model/blob/main/README.md#handling-missing-classes-in-target-column-with-regressor)). Later, we thought that maybe it was a good idea to just delete rows with missing `sii`. After couple of test, where we were using the same model just handling missing `sii` differently the best method turned out to be just deleting these rows. And we stuck with that `till the end of the project.
